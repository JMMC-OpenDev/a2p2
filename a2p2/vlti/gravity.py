...
from a2p2.vlti.instrument import ConstraintSet
        ...
        try:
            ...
            for res in self.getRange("GRAVITY_gen_acq.tsf", "INS.SPEC.RES"):
                if res in instrumentMode[0:len(res)]:
                    acqTSF.INS_SPEC_RES=res                   
            if "COMBINED" in instrumentMode:
                acqTSF.INS_FT_POL = 'OUT'
                acqTSF.INS_SPEC_POL = 'OUT'
            else:    
                acqTSF.INS_FT_POL = 'IN'
                acqTSF.INS_SPEC_POL =  'IN'
            ...
            for observationConfiguration in ob.observationConfiguration:        
                ...
                # TODO We should copy new keyword storage to forget previous loop values
                ...
                target.name = scienceTarget.name
                acqTSF.SEQ_INS_SOBJ_NAME = scienceTarget.name
                ...
                if acqTSF.SEQ_INS_SOBJ_MAG  < skyTransparencyMagLimits[tel]:
                    cs.setSkyTransparency('Variable, thin cirrus')
                else:
                    cs.setSkyTransparency('Clear')
                ...
                cs.setSeeing(1.0)
                cs.setBaseline( BASELINE.replace(' ', '-') )
                ...
                if dryMode:
                    ...
                    self.createGravityOB(ui, self.facility.a2p2client.getUsername(), api, containerId, acqTSF, cs, OBJTYPE, BASELINE, instrumentMode, target, DIAMETER, COU_AG_GSSOURCE, GSRA, GSDEC, COU_GS_MAG, dualField, SEQ_FT_ROBJ_NAME, SEQ_FT_ROBJ_MAG, SEQ_FT_ROBJ_DIAMETER, SEQ_FT_ROBJ_VIS, LSTINTERVAL)
...
#            ui.ShowErrorMessage("Value error :\n %s \n%s\n\n%s" % (e, trace, "Aborting submission to P2. Look at the whole traceback in the log."))
            ui.ShowErrorMessage("Value error :\n %s \n\n%s" % (e, "Aborting submission to P2. Look at the whole traceback in the log."))
            trace = traceback.format_exc()
            ui.addToLog(trace, False)
            ui.setProgress(0)
    ...
    def createGravityOB(self,ui, username, api, containerId, acqTSF, constraintSet, OBJTYPE, BASELINE, instrumentMode, target, 
        ...
        goodName = re.sub('[^A-Za-z0-9]+', '_', target.name.strip())
        ...
        # copy target info
        targetInfo = target.getDict()
        for key in targetInfo:
            ob['target'][key] = targetInfo[key]

        #copy constraints
        constraints = constraintSet.getDict() 
